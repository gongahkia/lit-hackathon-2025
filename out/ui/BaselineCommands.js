...
class BaselineCommands {
    constructor(dataService, diagnosticProvider) {
    ...
    async showFeatureDetails(featureOrId) {
    ...
    showDocumentReport(fileName, findings) {
        const panel = vscode.window.createWebviewPanel('groundworkDocumentReport', `GroundWork Report: ${fileName.split('/').pop()}`, vscode.ViewColumn.Beside, { enableScripts: true });
